...
interface BenefitEncryptionDataInput {
	trandata: string
	key?: string
	iv?: string
}

export function encrypt(data: BenefitEncryptionDataInput) {
...
interface BenefitDecryptionDataOutput {
	date: string
	authRespCode: string
	authCode: string
	tranId: number
...
interface BenefitDecryptionDataInput {
	buffer: Buffer
	key?: string
	iv?: string
}
...
export function decrypt(
	data: BenefitDecryptionDataInput,
): BenefitDecryptionDataOutput {
	const iv = BENEFIT_IV || data.iv
	const key = BENEFIT_TERMINAL_RESOURCE_KEY || data.key
	...
	if (!encryptedHex) throw new Error('No trandata found in the buffer')
